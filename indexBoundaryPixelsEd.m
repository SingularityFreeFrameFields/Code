function [BoundaryPixelInd, numBoundaries, NFour, smallLenBoundaryInd, BoundaryPixelIndold, Nold, narrowBand] = indexBoundaryPixelsEd(M, narrowBand)
%% Docstring
...
% Going with 8-connectivity for interiors, and detecting from there
% Thus, 4-connectivity for exteriors
...
% VISUALIZATION
%        [B,L,N,A] = bwboundaries(narrowBand, 4);
%        figure; 
%        imshow(narrowBand); 
%        hold on;
%        colors = ['b' 'g' 'r' 'c' 'm' 'y'];
...
% Exterior processing above, now let's just do the interior processing
for k = NFour+1:length(B)
    boundary = B{k};
    %BoundaryPixelInd{k} = sub2ind(size(narrowBand),boundary(:,1), boundary(:,2));
...
%     end   
    ...
    boundaryInd = sub2ind(size(narrowBand),boundary(:,1), boundary(:,2));
    
    [~,topLeft] = min(boundaryInd);
    
    r1 = boundary(topLeft,1);
    c1 = boundary(topLeft,2);
    
    % counterclockwise implies clockwise for interior boundaries;
    % essentially it means keep empty cells on your left if 'clockwise' and
    % empty cells on your right if 'counterclockwise'
    C = bwtraceboundary(narrowBand, [r1, c1-1], 'n', 4, inf, 'counterclockwise');
...
% Just assigning random values
numBoundaries = length(BoundaryPixelInd);
NEight = NFour;
Nold = NEight;
smallLenBoundaryInd = 0;
BoundaryPixelIndold = BoundaryPixelInd;
